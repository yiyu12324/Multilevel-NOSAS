...
# ====================== Configuration ======================
n_subdomains_list = [128,16]  # 3-level: L0=128 L1=16 L2=FinalLevel
mu_list = [0.5,0.5,0.5]          # mu per level
eta_list = [0.7]              # Section 4 filter threshold
...
def build_nosas_global_operator(A_QG_data, D_inv_G, InvS_GG, GG, n_total):
    ...
    if n_eigen > 0 and n_gg > 0:
        ...
        A_QG_csr = csr_matrix((A_vals, (A_rows, A_cols)), shape=(n_total, n_eigen))

        GG_arr = np.asarray(GG)
        A_GG_csr = A_QG_csr[GG_arr, :]

        I_i, I_j, I_v = InvS_GG.getValuesCSR()
        InvS_GG_csr = csr_matrix((I_v, I_j, I_i), shape=InvS_GG.getSize())

        D_vals = np.concatenate([d for d in D_inv_G if d is not None and len(d) > 0])

        W_csr = InvS_GG_csr @ A_GG_csr
        GS_csr = diags(D_vals, shape=(n_eigen, n_eigen)) - A_GG_csr.T @ W_csr

        W = PETSc.Mat().createAIJ(
            size=W_csr.shape,
            csr=(W_csr.indptr.astype(PETSc.IntType),
                 W_csr.indices.astype(PETSc.IntType),
                 W_csr.data.astype(PETSc.ScalarType)),
        ...
        Global_Solver = PETSc.Mat().createAIJ(
            size=GS_csr.shape,
            csr=(GS_csr.indptr.astype(PETSc.IntType),
                 GS_csr.indices.astype(PETSc.IntType),
                 GS_csr.data.astype(PETSc.ScalarType)),
            comm=PETSc.COMM_WORLD
        )
...
def _eps_krylov_schur(T_shell, L_blocks, comp_dofs, mu_val, m_size, col_offset, nev_cap=None):
    ...
    Returns (Q, lam_list) with B-orthonormal Q columns and lam < mu."""
...
# ====================== Section 4 filter (block Cholesky) ======================
def section4_filter(A_QG_sparse, B_Gamma, D_inv, s, eta, use_invD, level_name='', group_id=-1):
    n_eigen_old = A_QG_sparse.shape[1]
    if isinstance(B_Gamma, np.ndarray):
        A_QG_np = A_QG_sparse.toarray() if hasattr(A_QG_sparse, 'toarray') else A_QG_sparse
        L = cholesky(B_Gamma, lower=True)
        W_mat = solve_triangular(L, A_QG_np, lower=True)
        WtW = W_mat.T @ W_mat
    else:
        B_sp = (B_Gamma + B_Gamma.T) / 2
        n_blocks, labels = connected_components(B_sp, directed=False)
        blocks = []
        for b in range(n_blocks):
            idx = np.where(labels == b)[0]
            B_block = B_sp[np.ix_(idx, idx)].toarray()
            B_block = (B_block + B_block.T) / 2
            L_block = cholesky(B_block, lower=True)
            blocks.append((idx, L_block))
        WtW = np.zeros((n_eigen_old, n_eigen_old))
        for idx, L_block in blocks:
            A_QG_block = A_QG_sparse[idx, :].toarray()
            W_block = solve_triangular(L_block, A_QG_block, lower=True)
            WtW += W_block.T @ W_block
    if use_invD:
    ...
    hat_U_s = A_QG_sparse.dot(WtW_inv_z)
    hat_U_s = hat_U_s @ np.diag(D_x)
...
R0T_level0_rows, R0T_level0_cols, R0T_level0_data = [], [], []
...
def compute_coarse_level(prev_S_Blk_rows, prev_S_Blk_cols, prev_S_Blk_vals,
                          ...
                          eta_val=None, level_name='', use_krylov=False, krylov_nev_cap=None):
    n_c = cur_data['n_c']
    ...
    for cid in range(n_c):
        ...
        if len(D_inv_child_list) == 0:
            D_inv_child_vec = np.array([], dtype=PETSc.ScalarType)
            D_child_vec = np.array([], dtype=PETSc.ScalarType)
        ...
        # ====================== Section 4 filtering (exact 2D pattern) ======================
        if eta_val is not None and col_offset > 0 and len(s) > 0:
            B_Gamma = S_Blk_local_csr[np.ix_(s, s)]
            B_Gamma = (B_Gamma + B_Gamma.T) / 2
            A_QG_np_s = A_QG_local_csr[s]
            if A_QG_np_s.size > 0 and A_QG_np_s.shape[1] > 0:
                use_invD = True
                hat_U_s, n_selected, _ = section4_filter(
                    A_QG_np_s, B_Gamma, D_inv_child_vec, s, eta_val, use_invD, level_name, cid)
                if n_selected > 0:
                    new_rows = np.repeat(s, n_selected)
                    new_cols = np.tile(np.arange(n_selected), len(s))
                    new_vals = hat_U_s.ravel().astype(PETSc.ScalarType)
                    A_QG_local_csr = csr_matrix((new_vals, (new_rows, new_cols)), shape=(n_dofs_total, n_selected))
                    A_QG_local = PETSc.Mat().createAIJ(
                        size=(n_dofs_total, n_selected),
                        csr=(A_QG_local_csr.indptr.astype(PETSc.IntType),
                             A_QG_local_csr.indices.astype(PETSc.IntType),
                             A_QG_local_csr.data.astype(PETSc.ScalarType)),
                        comm=PETSc.COMM_WORLD)
                    ...
                    # After filtering, D = I (hat_U absorbs eigenvalue weights)
                    D_inv_child_vec = np.ones(n_selected, dtype=PETSc.ScalarType)
                    D_child_vec = np.ones(n_selected, dtype=PETSc.ScalarType)
                    indices_d = np.arange(n_selected, dtype=np.int32)
                    D_local_csr = csr_matrix((D_child_vec, (indices_d, indices_d)), shape=(n_selected, n_selected))
                    D_inv_csr = csr_matrix((D_inv_child_vec, (indices_d, indices_d)), shape=(n_selected, n_selected))
                    D_inv_petsc = PETSc.Mat().createAIJ(
                        size=(n_selected, n_selected),
                        csr=(D_inv_csr.indptr.astype(PETSc.IntType),
                             D_inv_csr.indices.astype(PETSc.IntType),
                             D_inv_csr.data.astype(PETSc.ScalarType)),
                        comm=PETSc.COMM_WORLD)
                    ...
                    A_QG_local_csr = csr_matrix(([], ([], [])), shape=(n_dofs_total, 0))
                    A_QG_local = PETSc.Mat().createAIJ(size=(n_dofs_total, 0),
        ...
        if use_krylov:
            ...
            if len_m == 0:
                ...
                T_shell.assemble()
                # nev budget (per user): L1 → ~m/3, L2 → 500
                if krylov_nev_cap is None:
                    local_nev_cap = max(len_m // 3, 20)     # L1: ~m/3
                else:
                    local_nev_cap = krylov_nev_cap         # L2: 500
                local_nev_cap = min(local_nev_cap, max(1, len_m - 2))
            ...
            if len_m > 0:
                ...
                Schur = ((Schur + Schur.T) / 2).tocsr()

                # 局部构造 A1_mat = Schur ∘ TS_Blk（无需全局 TS_Blk）
                A1_mat = build_A1_masked(Schur, Subd_E, Subd_F, m)
                n_eigen_k = min(200, max(1, len_m - 2))
                Schur_dense = Schur.toarray()
                A1_mat_dense = A1_mat.toarray()
                D_full, Q_full = eigh(Schur_dense, A1_mat_dense)
                n_eigen_k = min(n_eigen_k, len(D_full))
                D, Q = D_full[:n_eigen_k].copy(), Q_full[:, :n_eigen_k].copy()
                del Schur_dense, A1_mat_dense, D_full, Q_full
                gc.collect()

                # Save S_Blk (A1_mat values)
                A1_csr = csr_matrix(A1_mat)
                indptr, indices, values = A1_csr.indptr, A1_csr.indices, A1_csr.data
                row_counts = np.diff(indptr)
                row_indices_local = np.repeat(np.arange(len(row_counts)), row_counts)
                ...
                S_Blk_vals[cid] = values            # R0T = -A2^T * yy (negative)
                yy_A3 = yy_csr @ A3
...
last_Subd_E = []
last_Subd_F = []
last_S_rows_lvl = None
last_S_cols_lvl = None
last_S_vals_lvl = None
last_A_QG_lvl = None
last_D_inv_lvl = None
last_level_was_skipped = False
...
for level_idx in range(n_coarse_levels):
    ...
    print(f"\n=== Computing {level_name} ({data['n_c']} groups, eta={eta_val}) ===")
    print(f"  >>> MU_USED = {data['mu']}  (mu_list={mu_list})")

    if data['n_c'] == 1:
        print(f"  {level_name}: single group, skipping")
        last_level_was_skipped = True
        continue
    ...
    Local_Solver_lvl, A_QG_data_lvl, flag_lvl, \
        ...
        compute_coarse_level(
            ...
            eta_val=eta_val, level_name=level_name,
            use_krylov=False,   # L1 uses dense eigh (41744289-style); KS was too slow for eta>=0.5
            krylov_nev_cap=(None if level_idx == 0 else 500))
    ...
    # Build R_0T from level 0 data (matching 2D pattern)
    ...
    if krylov_r0t_groups is not None:
        # Krylov-Schur levels: implicit R_0T = -yy·A3 (shell mat, never n×m dense)
    ...
    last_coarse_GG = data['GG']
    last_Subd_E = data['Subd_E']
    last_Subd_F = data['Subd_F']
    last_S_rows_lvl = S_Blk_rows_lvl
    last_S_cols_lvl = S_Blk_cols_lvl
    last_S_vals_lvl = S_Blk_vals_lvl
    last_A_QG_lvl = A_QG_data_lvl
    last_D_inv_lvl = D_inv_G_lvl
...
# ====================== Final filter on last level (only when 1-group skipped) ======================
if last_level_was_skipped and last_coarse_GG is not None and len(last_coarse_GG) > 0:
    n_final = len(last_A_QG_lvl)
    s_arrays_m = [d['m'] for d in last_A_QG_lvl if d is not None and d['n_eigen'] > 0]
    if len(s_arrays_m) == 0:
        s_all = np.array([], dtype=np.int32)
    else:
        s_all = np.unique(np.concatenate(s_arrays_m))
    all_SR = np.concatenate([last_S_rows_lvl[i] for i in range(n_final) if last_S_rows_lvl[i] is not None and len(last_S_rows_lvl[i]) > 0])
    all_SC = np.concatenate([last_S_cols_lvl[i] for i in range(n_final) if last_S_cols_lvl[i] is not None and len(last_S_cols_lvl[i]) > 0])
    all_SV = np.concatenate([last_S_vals_lvl[i] for i in range(n_final) if last_S_vals_lvl[i] is not None and len(last_S_vals_lvl[i]) > 0])
    aR, aC, aV = [], [], []; co = 0
    for i in range(n_final):
        d = last_A_QG_lvl[i]
        if d is None or d['n_eigen'] == 0: continue
        aR.append(np.repeat(d['m'], d['n_eigen'])); aC.append(np.tile(np.arange(d['n_eigen']) + co, len(d['m'])))
        aV.append(d['A1Q'].ravel()); co += d['n_eigen']
    if co > 0 and len(s_all) > 0:
        print(f"\n=== Final filtering ({len(s_all)} DOFs, {co} modes) ===")
        # Triplet filtering: avoid Sc[np.ix_(s_all)].toarray() OOM
        remap = np.full(n_dofs_total, -1, dtype=np.int32)
        remap[s_all] = np.arange(len(s_all))
        keep = (remap[all_SR] >= 0) & (remap[all_SC] >= 0)
        loc_R = remap[all_SR[keep]]
        loc_C = remap[all_SC[keep]]
        Bf = csr_matrix((all_SV[keep], (loc_R, loc_C)), shape=(len(s_all), len(s_all)))
        keep_SR = all_SR[keep]; keep_SC = all_SC[keep]; keep_SV = all_SV[keep]
        del remap, keep, loc_R, loc_C
        Af = np.concatenate(aV); Arf = np.concatenate(aR); Acf = np.concatenate(aC)
        # 直接按 s_all 局部标号构造（无需全局 shape 再切片）
        smap = np.full(n_dofs_total, -1, dtype=np.int32)
        smap[s_all] = np.arange(len(s_all))
        keep_a = smap[Arf] >= 0
        Anp = csr_matrix((Af[keep_a], (smap[Arf[keep_a]], Acf[keep_a])),
                         shape=(len(s_all), co))
        Di = np.concatenate([last_D_inv_lvl[i] for i in range(n_final) if last_D_inv_lvl[i] is not None and len(last_D_inv_lvl[i]) > 0])
        hu, ns, _ = section4_filter(Anp, Bf, Di, s_all, eta_list[-1], True, 'FinalLevel', 0)
        print(f"  Final: {ns} EVs (from {co})")
        if ns > 0:
            fAQ = [{'m': s_all, 'A1Q': hu, 'n_eigen': ns}]
            fSR = [keep_SR]; fSC = [keep_SC]; fSV = [keep_SV]
            ISn, _, _, _ = build_InvS_Blk_on_GG(fSR, fSC, fSV, last_Subd_E, last_Subd_F, last_coarse_GG)
            fDI = [np.ones(ns, dtype=PETSc.ScalarType)]
            Wn, GSn = build_nosas_global_operator(fAQ, fDI, ISn, last_coarse_GG, n_dofs_total)
            all_InvS_Blk[-1] = ISn; all_W[-1] = Wn; all_Global_Solver[-1] = GSn
...
if n_coarse_levels > 0:
    ...
    # Build R_0T from level 0 data (matching 2D)
    R0T_lvl0_rows = np.concatenate(R0T_level0_rows) if R0T_level0_rows else np.array([], dtype=np.int32)
    R0T_lvl0_cols = np.concatenate(R0T_level0_cols) if R0T_level0_cols else np.array([], dtype=np.int32)
    R0T_lvl0_vals = np.concatenate(R0T_level0_data) if R0T_level0_data else np.array([], dtype=PETSc.ScalarType)
    R_0T_csr = csr_matrix((R0T_lvl0_vals, (R0T_lvl0_rows, R0T_lvl0_cols)), shape=(n_dofs_total, n_dofs_total))
    R_0T = PETSc.Mat().createAIJ(size=(n_dofs_total, n_dofs_total),
                                 csr=(R_0T_csr.indptr.astype(PETSc.IntType),
                                      R_0T_csr.indices.astype(PETSc.IntType),
                                      R_0T_csr.data.astype(PETSc.ScalarType)))
    R_0T = R_0T.createSubMatrix(is_II0, is_GG0)
    R_0T.assemble()
