...
def section4_filter(A_QG_np_s, S_Blk_s_sparse, D_inv, eta, use_invD,
                    level_name='', group_id=-1):
    n_eigen_old = A_QG_np_s.shape[1]

    n_blocks, labels = connected_components(S_Blk_s_sparse, directed=False)

    blocks = []
    for b in range(n_blocks):
        idx = np.where(labels == b)[0]
        B_block = S_Blk_s_sparse[np.ix_(idx, idx)].toarray()
        B_block = (B_block + B_block.T) / 2
        L_block = cholesky(B_block, lower=True)
        blocks.append((idx, L_block, B_block))

    def tri_solve_blocks(v, lower=True):
        result = np.zeros_like(v)
        for idx, L_block, _ in blocks:
            result[idx] = solve_triangular(L_block, v[idx], lower=lower)
        return result

    def tri_T_solve_blocks(v):
        result = np.zeros_like(v)
        for idx, L_block, _ in blocks:
            result[idx] = solve_triangular(L_block.T, v[idx], lower=False)
        return result

    def B_mul_blocks(v):
        result = np.zeros_like(v)
        for idx, _, B_block in blocks:
            result[idx] = B_block @ v[idx]
        return result

    W_mat = tri_solve_blocks(A_QG_np_s, lower=True)
    WtW = W_mat.T @ W_mat
    if use_invD:
    ...
    tmp = np.linalg.solve(WtW, z)
    Wt_tmp = W_mat @ tmp
    xi = tri_T_solve_blocks(Wt_tmp)
    hat_U_s = B_mul_blocks(xi) @ np.diag(D_x)
    ...
    for coarse_id in range(n_coarse_subdomains):
        ...
        if n_eigen_old == 0 or len(s) == 0:
            ...
            hat_U_s, n_selected, D_inv_s = section4_filter(
                A_QG_np_s, S_Blk_s_sparse, D_inv_local, eta, use_invD, level_name, coarse_id)
...
def build_InvS_Blk_3d(Subd_E_in, Subd_F_in, S_Blk_petsc_in):
    ...
    for entity in Subd_F_in + Subd_E_in:
        entity = entity.astype(PETSc.IntType)
        is_entity = PETSc.IS().createGeneral(entity, comm=PETSc.COMM_WORLD)
        S_sub = S_Blk_petsc_in.createSubMatrix(is_entity, is_entity)
        S_sub.assemble()
        I_s, J_s, V_s = S_sub.getValuesCSR()
        S_dense = csr_matrix((V_s, J_s, I_s), shape=S_sub.getSize()).toarray()
        U_inv = np.linalg.inv(np.linalg.cholesky(S_dense))
        inv = U_inv.T @ U_inv
        n = len(entity)
        rows.extend(np.repeat(entity, n)); cols.extend(np.tile(entity, n)); vals.extend(inv.ravel())
        is_entity.destroy()
        S_sub.destroy()
...
def assemble_final_global_3d(A_QG_data, S_Blk_rows, S_Blk_cols, S_Blk_vals,
    ...
    InvS_Blk_full = build_InvS_Blk_3d(Subd_E_list, Subd_F_list, S_Blk_full)
...
InvS_full = build_InvS_Blk_3d(Subd_E, Subd_F, S_Blk_petsc)
